...
\email{chrzaszcz.marcin@gmail.com}
\author{Nicola Serra}
...
that the variates $x^{(k)}$, $k = 1, \dots, N$ are distributed as
$P$; i.e.,
\begin{equation}
    x^{(k)} \sim P\,.
\end{equation}
...
P_i\rangle$\footnote{% We refer for example to the determination of
  the angular observables $J_i$ in $\bar{B}\to\bar{K}^*\ell^+\ell^-$
  decays, see e.g. \cite{Aaij:2013iag}. For details on the relation
  between the $P_i$ and the angular observable $J_i$ in the
...
\begin{equation}
    ...
    = \frac{1}{N} \sum_{k=1}^{N} f_i(x^{(k)})         \mathbf{1}(\vec{a} \le \nuvec \le \vec{b})\,.
\end{equation}
...
\begin{equation}
    \cov[\aver{P_i},\aver{P_j}]_{\vec{a},\vec{b}} \to \what{\cov}[\aver{P}_i, \aver{P}_j]_{\vec{a},\vec{b}}
        = \frac{1}{N - 1} \sum_{k=1}^{N} \big[{f}_i(x^{(k)}) - \widehat{\langle P_i\rangle_{\vec{a},\vec{b}}}\big]\,\big[f_j(x^{(k)}) - \widehat{\langle P_j\rangle_{\vec{a},\vec{b}}}\big]
        \mathbf{1}(\vec{a} \le \nuvec \le \vec{b})\,.
\end{equation}
We would like to emphasize that the sample covariance should rapidly \fred{So what value of $N$ is close enought to $\infty$?} converge towards the true covariance matrix.
...
\section{On the Partial Wave Expansions of Interference Terms}
...
\begin{equation}
    P(\theta) \equiv A_1(\theta) A_2^*(\theta) \equiv \left(\sum_{l_1=0}^\infty A_1^{(l_1)} p_{l_1}^{(m_1)}(\cos\theta)\right) \left(\sum_{l_2=0}^\infty A_1^{(l_2)} p_{l_2}^{(m_2)}(\cos\theta)\right)\,.
\end{equation}
Here $m_1$ and $m_2$ denote the third component of the angular momentum of the respective amplitude $A_1$ or $A_2$, and we impose $l_1 \geq l_2$.\\
...
\end{equation}
Using the large-$l$ behaviour of $n_{l_i,m_i}$ one finds that with increasing $l_i$ the partial waves amplitudes must asymptotically fall off,
\begin{equation}
    |A_i^{(l_i)}(\cos\theta)| \Big|_{l_i \gg m_i} \sim \frac{1}{\sqrt{l_i}}\,.
\end{equation}
This motivates to cut-off of the partial-wave expansion at some angular momentum $L$. Will will show in the following that such a cut-off
is compatible with defining a basis of angular observables as coefficients of Legendre polynomials in $\cos\theta$.\\

We propose the decomposition of $P$ in terms of the associated Legendre polynomials $p_{j}^{(m)}(\cos\theta)$, with $m=m_1 + m_2$, and
$l_1 - l_2 \leq j \leq l_1 + l_2$,
\begin{equation}
...
\begin{equation}
    ...
    \begin{aligned}
        P_{j,m}(\vec{\nu})
            & = \frac{1}{n_{j,m}} \int_{-1}^{+1} p_{j}^{(m_1 + m_2)}(\cos\theta) \sum_{l_1,l_2=0}^\infty A_{1}^{l_1} p_{l_1}^{(m_1)}(\cos\theta) \big(A_{2}^{(l_2)}\big)^* p_{l_2}^{(m_2)}(\cos\theta)\\
            & = \sum_{l_1,l_2=0}^\infty A_{1}^{l_1} \big(A_{2}^{(l_2)}\big)^* \frac{T_{l_1,l_2,j}^{(m_1,m_2)}}{n_{j,m}}\,,\qquad\text{with }m = m_1 + m_2\,.
    \end{aligned}
\end{equation}
...
\end{equation}
The latter condition is well known from adding angular momenta.
