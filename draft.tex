...
It is sensible to refer to the angular observable $S_i$ as the \emph{$f_i$-moment} of the PDF $P$.
...
\end{equation}
This reduces the estimation of $\aver{S_i}_{\vec{a},\vec{b}}$ to a
(weighted) counting experiment. In the limit $N \to \infty$, the
central limit theorem (CLT) implies that the random vector
\begin{equation}
  \label{eq:angular-obs-vec}
  \widehat{\vec{S}_{\vec{a},\vec{b}}} \equiv (\aver{S_1}_{\vec{a},\vec{b}}\,, \dots, \aver{S_i}_{\vec{a},\vec{b}}\,,
  \dots)
\end{equation}
 follows a multivariate Gaussian distribution whose mean vector
is approximated by \refeq{bin-importance} and whose covariance is
estimated as
\begin{equation}
    \cov[\aver{S_i},\aver{S_j}]_{\vec{a},\vec{b}} \to \est{\cov}[\aver{S}_i, \aver{S}_j]_{\vec{a},\vec{b}}
        = \frac{1}{K - 1} \sum_{k=1}^{K} \big[\dual{f}_i(x^{(k)}) - \est{\langle S_i\rangle_{\vec{a},\vec{b}}}\big]\,\big[\dual{f}_j(x^{(k)}) - \widehat{\langle S_j\rangle_{\vec{a},\vec{b}}}\big]
        \mathbf{1}(\vec{a} \le \nuvec \le \vec{b})\,.
\end{equation}
...
\begin{equation}
    \est{\operatorname{Skew}}[\aver{S_i},\aver{S_j}] \leq B_S\,\,\text{ and }\,\,\est{\kurt}[\aver{S_i},\aver{S_j}] \leq B_K\,.
\end{equation}
...
\begin{enumerate}
\item The angular observable $\aver{S_i}$ can be determined
  independently of any other observable $\aver{S_j}$. It is therefore
  ...
  specific regarding the form of new-physics contributions.
...
\item The approximate normal distribution allows easier and more
  correct transfer of the information in the data to interested
  theorists for more accurate fits of standard-model and new-physics
  parameters \citeneeded\fred{Our fits and others'?} or for more precise predictions of
  optimized observables; e.g.,
  ...
  theorists if $\widehat{\vec{S}_{\vec{a},\vec{b}}}$ is not well
  ...
  phase-space or unitarity arguments as \fred{Could add a sketch. It
    is supposed to resemble $A_{\text{FB}},F_L$ for $B \to K^* \ell \ell$.}
  \begin{equation}
    \label{eq:constr-ex}
    |S_1| + S_2 \le 1, \, S_1 \in [-1,1],\, S_2 \in [0,1] \,.
  \end{equation}
  It may (and often does) happen in practice that
  $\widehat{\vec{S}_{\vec{a},\vec{b}}}$ is close or even outside the
  allowed region such that a significant part of the probability mass
  of $\vec{S}$ covers unphysical values. In a Bayesian fit, one would
  take $\mathcal{N}(\vec{S} | \widehat{\vec{S}_{\vec{a},\vec{b}}},
  \widehat{\Sigma})$ as the sampling distribution of the ``data'', and
  simply set a uniform prior on the triangle defined by
  \refeq{constr-ex} to have a well defined problem. This could be
  trivially combined with other independent information in a global
  fit. For a likelihood fit, the constraint needs to be built in by
  the experiment into the analysis of the actual events and the
  resulting likelihood as a function of $\vec{S}$ is distinctly not
  Gaussian. Since experimental collaborations as a whole tend to favor
  data-privacy over accurate use of their results in theorist's
  analyses, the multivariate likelihood is kept secret, and cannot be
  recovered from the numerical value of $\vec{S}$ and the standard
  errors reported in written publications. The theorist then usually
  includes the results as independent measurements with a Gaussian
  distribution and disregards the constraint altogether. This last
  situation is undesirable for both experimentalists and theorists.
  \danny{Two comments:
      \begin{itemize}
      \item We should tune down the ``data privacy'' line of thought. LHCb
      actively works on providing the full likelihood to theorists.
      Rather, we should emphasize that providing the likelihood
      as obtained from the ML-fit is technically much harder (think: profile likelihood,
      since the theorists are not interested in control channels, \dots)
      \item The constraints as described here are \emph{theoretical}
      in nature! If one changes the PDF (e.g. in $B\to K^*\ell^+\ell^-$,
      higher partial waves will have an impact on the shape of the
      \emph{physical region}. Thus, imposing these constraints should
      not be done on the side of the experiment.
      \end{itemize}
  }
\end{enumerate}
