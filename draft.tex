...
following is general and applies to \add{all decay or scattering
processes that can be formulated in terms of an orthogonal basis of angular functions.}
\fred{Not really arbitrary, it only works with orthogonal
basis functions.} \danny{Is this OK?}
...
\item The estimate is unbiased for any $N$. In contrast, the
  ...
  fits --- if they converge --- produce uncertainties \add{(10--15)\%}
      smaller than those from the method of moments  \fred{Is this true, Marcin?} \danny{Marcin already confirmed!}.
...
should be able to distinguish these based on context.}
which reads
\begin{equation}
    \label{eq:expansion}
