...
following is general and applies to arbitrary decay or scattering
processes \fred{Not really arbitrary, it only works with orthogonal
  basis functions}. We find a previous work \cite{Dighe:1998vk} that
...
\fred{For us, the major advantage is that the experiment can obtain all angular observables at once, and if there are more, one doesn't need more data. I'm not sure this becomes clear from the text, point 1.}
...
are equivalent for both approaches \fred{I'm confused. What are the two approaches? Do we consider Legendre polynomials as an orthonormal basis or not?} provided that one replace $f_i \to
...
\item The estimate is unbiased for any $N$. In contrast, the
  ...
  unknown parameter. \add{One should therefore not prefer the method of moments
  ...
  fits --- if they converge --- produce uncertainties \add{(10--15)\%}
  smaller than those from the method of moments  \fred{Is this true, Marcin?}.
...
Using the observables $S_k(\vec{\nu})$, $k=1,\dots,9$, with $\vec{\nu}=(q^2, k^2)$ \fred{It not clever to use $k$ as an index and momentum variable}
...
\begin{equation}
    ...
    \end{cases}\,.
\end{equation}
The angular observables $S_{k,j}$ defined as coefficients of this
expansion in Legendre polynomials have the merit of a well defined
total angular momentum, and thus are physically distinguishable.  As a
consequence of the orthogonality of the Legendre polynomials, any
mismodeling (or rather, lack of modeling of higher partial-wave
observables) \emph{does not affect} the method of moments as discussed
in the previous section. \add{That is to say}, ignoring further
(orthogonal) terms in the PDF neither affects $\vecest{S}$ nor its
variance.
...
\del{The precise value of $R$ (see \refeq{def-R}) depends on the a priori
unknown angular observables $\vec S$.  However, within ratios and
other suitable combinations of the estimators $\vecest{S}$, the quantity
$R$ cancels.
Conversely, $R$ can be
...
samples \fred{Before or after thinning?} (i.e., $2 \cdot 10^6$ MC samples per row of $M_{ij}$), we find that the error on the mean is
...
\begin{itemize}
    \item In all studied cases we observed not a single bias in the distribution of the $\mathrm{pull}$
        ...
        \end{equation}
        \danny{Marcin, please check that this is what you used!}
