...
\begin{equation}
    P(q^2, \cos\theta_1, \cos\theta_2, \theta_3) = \frac{1}{\dd\Gamma/\dd q^2} \frac{\dd^4\Gamma}{\dd q^2\,\dd \cos\theta_1\,\dd \cos\theta_2\,\dd \theta_3} = \frac{3}{8\pi} \sum_k \frac{J_{k}(q^2)}{\dd\Gamma/\dd q^2} f_k(\cos\theta_1, \cos\theta_2, \theta_3)\,,
\end{equation}
...
\end{equation}
where we also define $S_k(q^2) \equiv J_k(q^2) / (\dd\Gamma/\dd q^2)$.\\
...
such that
\begin{equation}
    \int_0^\pi \dd\theta_1 \int_0^\pi\,\dd\theta_2\, \int_0^{2\pi}\dd \theta_3 \hat{f}_k(\theta_1,\theta_2,\theta_3) P(q^2, \theta_1,\theta_2,\theta_3) = S_k(q^2)\,.
\end{equation}
...
$S$ have an expansion in terms of partial waves $l_1, l_2 = 0,1,2,\dots \hat{=}$ S,P,D of the underlying amplitudes $A_1$ and $A_2$,
\begin{equation}
...
\end{equation}
where $F = \text{Re},\text{Im}$ denotes taking either the real or the imaginary part.
...
\begin{equation}
    ...
    \begin{aligned}
        S_{j,m}(\vec{\nu})
            & = \frac{1}{n_{j,m}} \int_{-1}^{+1} p_{j}^{(m_1 + m_2)}(\cos\theta) \sum_{l_1,l_2=0}^\infty F\left[A_{1}^{l_1} p_{l_1}^{(m_1)}(\cos\theta) A_{2}^{*(l_2)}p_{l_2}^{(m_2)}(\cos\theta)\right]\\
...
We close by remarking that for any angular momentum cut-off $\ell < L$, one can find an upper limit for the squared sum
of all remaining partial waves through the inclusive decay; i.e. schematically
\begin{equation}
    \sum_{i=1}^2 \sum_{l=L}^\infty |A_i^{(l)}|^2 \leq \frac{\dd^{1 + \dim\vec\nu} \Gamma_\text{incl}}{\dd \cos\theta\, \dd\,\vec\nu}\,.
\end{equation}
