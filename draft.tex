...
\begin{abstract}
  ...
  integrals by Monte Carlo techniques. \fred{The main advantage of
    ...
    implementation of the method for several rare decays currently of
    interest.}
\end{abstract}
...
\fred{the normalization constant $R$ is given by
\begin{equation}
    R \equiv \int \int P_\text{t}(x_\text{t}) E(x_\text{d} | x_\text{t}) \rmdx{x_\text{t}} \rmdx{x_\text{d}}\,,
\end{equation}
...
\end{equation}
where the PDF $P(\xd|\xt)$ includes the resolution effects and the
...
% For the sake
% of clarity, we will restrict ourselves to pure acceptance effects and
% suppress resolution effects in the remainder of this section; i.e.,
% \begin{equation}
%     E(x_\text{d}, x_\text{t}) \equiv \eps(x_\text{t}) \delta(x_\text{d}, x_\text{t})\,.
% \end{equation}
% We will refer to $\eps$ as the \emph{detector acceptance} function.
In what follows, we propose a systematic method to unfold all effects
of \fred{$\Ekernel$} through MC simulations and the method of moments
\fred{assuming that $\Ekernel$ is independent of the angular
...
\equiv P_k(x)$) \fred{and angular observables $\lbrace S_k \rbrace$ as
\begin{equation}
    P\subt(\xt) \equiv P\subt(\xt | \lbrace S_k \rbrace) =   \sum_k S_k f_k(\xt),
  \end{equation}}
where $k = 0, 1, \dots$ denotes the total angular momentum of the observables \fred{adjust ``angular momentum'' to language at the end of the section}.
\fred{Normalization of $P\subt$ is equivalent to choosing} $S_0 = 1/2$. \fred{Requiring $P\subt(\xt) \ge 0 \,\forall \xt$ implies} $|S_k| \leq 1/2$ for angular momentum \fred{$k$ just an index, can we remove ``angular momentum''?} $k > 0$. More stringent relations between the
...
and \fred{note} that
\begin{equation}
    P^{(m)}(\fred{\xt}) \equiv P(\fred{\xt} | \lbrace S^{(m)}_k \rbrace)
\end{equation}
is a \fred{valid} PDF. The dual basis of angular functions follows then from the normalization
...
\end{equation}
We now define the \emph{simulated raw} moments $\lbrace Q_{i}^{(m)}\rbrace$ as
\begin{equation}
    Q_i^{(m)} \equiv \fred{\int \int \tilde{f}_i(\xd) P^{(m)}(\xt) \Ekernel \rmdx{\xt} \rmdx{\xd},}
\end{equation}
which are instrumental to our recipe. \fred{Monte Carlo} estimators of these moments can be constructed from specifically crafted detector events $x_\text{d}^{(n,m)}$, $n = 1, \dots, N_\text{d}^{(m)}$, where
\begin{equation}
    x_\text{d}^{(n,m)} \sim \fred{P_E^{(m)}(\xd) \equiv \frac{1}{R^{(m)}} \int P^{(m)}(\xt) \Ekernel \rmdx{\xt}}\,.
  \end{equation}
  \fred{In words, for each $m$ it is required to generate events from
    a toy physical distribution in which $S_0 = S_m=1/2$ and all other
    observables are set to zero, and to propagate those events through
    a detector simulation.} The normalization $R^{(m)}$ is chosen such
  that \fred{$\int P_E^{(m)}(\xd) \rmdx{\xd} = 1$}.  We emphasize that
...
\end{equation}
We call the matrix $M_{ij}$ the \emph{unfolding matrix}, which is
specific to the decay at hand. Given our definition of $S_k^{(m)}$ in
\fred{\refeq{def-Sk} it is easy to see that}
\begin{equation}
    M_{ij} = \begin{cases}
        2 Q_i^{(0)}                          & j = 0\,,\\
...
unknown angular observables $\lbrace S_i\rbrace$.  However, ratios and
other suitable combinations of the estimators $\lbrace
\est{S}_i\rbrace$ are independent of $R$. Conversely, $R$ can be
...
\danny{
We note that the set $\lbrace \est{S}_k\rbrace$ as determined from \refeq{estS} does not fully correspond to the
set $\lbrace S_k\rbrace$ for an arbitrary $\eps$. Taking again the decay $B\to K\ell^+\ell^-$
as an example, we have $\dim S = 3$. Assuming an expansion of the detector acceptance $\eps$
in terms of Legendre polynomial up to order $L$, we obtain $\dim \est{Q} = \dim \est{S} = \dim S + L$.
The MC estimators then have the following structure:
\begin{equation}
...
Since $L$ depends on the \fred{setup} of the particle detector \fred{under consideration}, it remains for
...
compute time and accuracy. \fred{For a uniangular distribution and a total of $6\cdot 10^6$ MC
...
ignored in the following. \fred{I rephrased it sligthly} Suppose $S$
...
off as \fred{Let's say why. Unitarity?} \danny{Finiteness of
  observables, so yes: unitarity!} \fred{So can't we just say
  unitarity before the first equation? That's the reason why the sum
  need be finite, I guess. But if I look at (D2) again, it goes down as $1/(2l + 1)$, so If remember correctly, $A_i^{(l_i)} \propto 1/l^{\eps}$ would be enough to ensure convergence. So why $1/\sqrt{l_i}$?}
\begin{equation}
