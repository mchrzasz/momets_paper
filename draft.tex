...
\end{equation}
\fred{since when is $S_k a fct. of \cos \theta_2$?}
...
\begin{equation}
  ...
  \end{equation}
  \fred{What happens to the uncertainties? Unfolding is used in many
    places, so there are elaborate recipes. Should we assume the same
    uncertainties as before? Since it is linear, rescaling should be sufficient. Or compute covariance directly for transformed observables} \danny{Here $N_\eps$ is a normalization factor
    based on the detector acceptance function $\eps$. It emerges since
    the product $P(x) \eps(x)$ is not a PDF; it is not normalized
    appropriately\footnote{We thank X.X\danny{Marcin: Please insert
        whatshisname} for pointing this out.}. We can determine
    $N_\eps$ by imposing
    \begin{gather}
...
\fred{I'm puzzled. If you can do the calculation to some order
  $\vec{l}_P$, why would you use a lower order for the actual result?
  I.e., if you have the NNLO results, why would you limit yourself to
  NLO?}
