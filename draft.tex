...
\begin{enumerate}
    \item \del{Likelihood fits have convergence problems for a small number of
    ...
    \item \add{Likelihood fits of problems, for which the underlying
        physical model is only partially known, may be delicate. Such cases can lead to
        overestimating the number of physical parameters, and consequently
        inhibit the convergence of the fits. (As an example, this type
    ...
    \item The results of likelihood fits suffer from systematic uncertainties
        and systematic bias \del{(see e.g. \cite{Egede:1142152})}, especially in but not limited
...
\item It is superior for a small number of samples $N$. Likelihood
  ...
  region.\fred{Is there some LHCb note we could cite?}\danny{Ulrik suggests citing some
  statistical paper}
  For some of these decays of interest, there are only $\order{10}$--$\order{100}$ events
  per bin on record.\danny{What to cite? 1403.8004 only states total signal yield!}
\item The estimate is unbiased for any $N$. In contrast, the
  ...
  unknown parameter. It is therefore highly questionable \add{to outright
  prefer the method of moments over likelihood fits, in order} to reduce
...
\item The approximate multivariate Gaussian distribution of $\vecest{S}$
    allows easier and more \add{precise}\del{correct} transfer of the information in the
...
Descotes-Genon:2013wba,Beaujean:2013soa},
  ...
  etc.) that are also part of the likelihood.
  \danny{Ulrik: Seems an odd comment to put in a paper.}
  This leads to the
...
with respect to the remaining angle $\theta_2$. \add{Here, the angular momentum-like indices $l$ and $m$ follows from the usual
...
\end{equation}
where the the normalization factor $n_{l,|m|}$ is defined in \refeq{legendre-scalar-product}. Within our
...
partial-wave observables) \emph{does not affect} the method of moments as discussed in the previous section. \del{To wit}\add{That is to say},
...
\add{Nevertheless, the expansion in terms of Legendre polynomials provides means to cope with these effects,
as we discuss in the following subsection.}
\del{The means to cope with this problem are discussed
...
\add{In this process, we use that fact} that $\Ekernel$ is independent of the angular
...
as an example, \del{we have} \add{there are exactly three physical angular observables:} $\dim S = 3$.
...
\add{We propose to do this by \dots}\\
...
$50$ to $500$ events. Our toy analyses base on SM predictions for angular observables
...
as well as Claus Grupen for helpful advice in searching the literatur.
